...
const packageRegistry =
  jsonContents.Package.Registry === "pip"
    ? "pypi"
    : jsonContents.Package.Registry;
...
if (!(packageRegistry === "other")) {
  ...
  const packageDetails = await graphQLClient
  ...
  if (packageDetails.package.url !== packageUrl) {
    core.warning(
      `Suggestion - should your Package.URL be: ${packageDetails.package.url} ?`
    );
    core.setFailed(`The PR does not contain a valid registry URL...`);
  }

  if (
    packageDetails.repository.owner !== repositoryOwner ||
    packageDetails.repository.name !== repositoryName
  ) {
    if (
      packageDetails.repository.owner === "n/a" &&
      packageDetails.repository.name === "n/a"
    ) {
      core.warning(
        `The repository could not be found using libraries.io. Please raise a support ticket: https://libraries.io/${packageRegistry}/${packageName}/suggestions`
      );
    } else {
      core.warning(
        `Suggestion - should your Repository.Owner and Repository.Name be: ${packageDetails.repository.owner}, ${packageDetails.repository.name}?`
      );
